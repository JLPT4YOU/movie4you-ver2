...
export default function WatchMovie({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const saveHistory = useCallback(async (overrides?: {
    ...
    // Save to localStorage for backwards compatibility and non-authenticated users
    ...
    if (user) {
      await watchProgressService.saveProgress({
        ...
        episode_name: source === 'mappletv' ? 'MappleTV' : (episode?.name || `Tập ${episodeIndex + 1}`),
        server_index: serverIndex,
        progress_seconds: overrides?.currentTime ?? currentPlaybackTime,
        duration_seconds: overrides?.duration ?? (videoDuration || 1),
      });
    }
  }, [movie, selectedSource, selectedServer, selectedEpisode, alternativeSources, currentPlaybackTime, videoDuration, user]);
  ...
  useEffect(() => {
    const fetchMovieData = async () => {
      ...
      try {
        ...
        const ophimRes = await fetch(`/api/ophim/v1/api/phim/${resolvedParams.slug}`);
        ...
        Promise.all([
          fetch(`/api/phimapi/${resolvedParams.slug}`).then(r => r.json()).catch(() => null),
          fetch(`/api/nguonc/${resolvedParams.slug}`).then(r => r.json()).catch(() => null)
        ]).then(([phimapi, nguonc]) => {
